...
class ProductSearchForm(FormValidationAction):
    ...
    def submit(
        ...
        if category == 'phone':
            cursor.execute("SELECT product_url, ram FROM products WHERE ram >= %s AND back_camera_megapixel >= %s AND \
                   battery_mah >= %s AND price_usd <= %s AND category=%s",[int(ram), int(camera), int(battery), int(budget),'phone'])
        elif category == 'laptop':
            cursor.execute("SELECT product_url, ram FROM products WHERE ram >= %s AND storage >= %s AND \
                   battery_backup >= %s AND price_usd <= %s AND category=%s",[int(ram), int(storage_capacity), int(battery_backup), int(budget),'laptop'])
